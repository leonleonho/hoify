...
describe("worker handler", () => {
  it("calls parseFile → identify → upsertOne → saveAlbumArt on success", async () => {
    ...
    const parsed = { filePath: "/music/song.mp3", title: "Song", artist: "Artist", album: "Album", genreNames: ["rock"] };
  ...
  it("does not call saveAlbumArt when no embedded picture", async () => {
    const parsed = { filePath: "/music/song.mp3", title: "Song", artist: "Artist", album: "Album", genreNames: [] };
    mockParseFile.mockResolvedValue(parsed);
